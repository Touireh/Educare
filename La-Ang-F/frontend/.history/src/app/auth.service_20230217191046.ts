...
export class AuthService {
  ...
  login(email: string, password: string) {
    return this.http.post(`${this.apiUrl}/utilisateur/login`, {email, password}).subscribe(
      (response: any) => {
        ...
        sessionStorage.setItem('name', response.user.name); // stocker le nom de l'utilisateur dans la session storage
  ...
  getUserName() {
    return sessionStorage.getItem('name'); // Retrieve the user's name from sessionStorage
  }
}